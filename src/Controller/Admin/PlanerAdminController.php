...
class PlanerAdminController extends AbstractController
{
    ...
    // ──────────────────────────────────────
    //  URLOPY FIRMA (heat-map miesięczny)
    // ──────────────────────────────────────

    #[Route('/urlopy-firma/{rok}/{miesiac}', name: 'planer_admin_urlopy_firma', methods: ['GET'],
        defaults: ['rok' => null, 'miesiac' => null],
        requirements: ['rok' => '\d{4}', 'miesiac' => '1[0-2]|[1-9]'])]
    public function urlopyFirma(
        ?int $rok,
        ?int $miesiac,
        ...
        return $this->render('@Planer/admin/urlopy_firma.html.twig', [
            'active' => 'urlopy',
            'rok' => $rok,
            'miesiac' => $miesiac,
    ...
    #[Route('/urlopy-firma/{rok}/{miesiac}/detail', name: 'planer_admin_urlopy_firma_detail', methods: ['GET'],
        requirements: ['rok' => '\d{4}', 'miesiac' => '1[0-2]|[1-9]'])]
    public function urlopyFirmaDetail(
        int $rok,
        int $miesiac,
        Request $request,
        PodanieUrlopoweRepository $podanieRepo,
        ...
        $targetDate = new \DateTime(sprintf('%04d-%02d-%02d', $rok, $miesiac, $dzien));
        $monthStr = sprintf('%04d-%02d', $rok, $miesiac);
